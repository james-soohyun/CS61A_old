...
def find_closest(location, centroids):
    ...
    for centroid in centroids:
        centroid_distances.append(distance(location, centroid))
    min_distance = min(centroid_distances)
    for i in range(len(centroids)):
        if distance(location, centroids[i]) == min_distance:
            return centroids[i]
    # END Question 3
...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        r_c_cluster.append([find_closest(restaurant_location(restaurant), centroids), restaurant])
    return group_by_first(r_c_cluster)
    # END Question 4
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        "*** YOUR CODE HERE ***"
        clustered_restaurants = group_by_centroid(restaurants, centroids)
        centroids = [find_centroid(cluster) for cluster in clustered_restaurants]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    S_xy = sum([(element[0] - mean_x)*(element[1] - mean_y) for element in xy])
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed:
            restaurant_ratings[restaurant_name(restaurant)] = user_rating(user, restaurant_name(restaurant))
        else:
            restaurant_ratings[restaurant_name(restaurant)] = predictor(restaurant)
